    ...
    <div class="py-12 bg-slate-50/50 min-h-screen">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-slate-100 p-8">
                ...
                <form action="{{ route('admin.operations.updateRecap', $trainingReport->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        <select name="coach_attendance" id="coach_attendance" required class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                            <option value="Hadir" {{ old('coach_attendance', $trainingReport->coach_attendance) === 'Hadir' ? 'selected' : '' }}>Hadir</option>
                            <option value="Tidak Hadir" {{ old('coach_attendance', $trainingReport->coach_attendance) === 'Tidak Hadir' ? 'selected' : '' }}>Tidak Hadir</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-6" id="student_attendance_section">
                        ...
                        <div class="bg-slate-50 rounded-lg p-4 border border-slate-200">
                            @foreach($schedule->students as $student)
                                @php
                                    $attendance = $trainingReport->studentAttendances->where('student_id', $student->id)->first();
                                    $status = old('student_attendance.'.$student->id, $attendance ? $attendance->status : 'Hadir');
                                    $evaluation = old('student_evaluations.'.$student->id, $attendance ? $attendance->evaluation : '');
                                @endphp
                                    ...
                                    <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-2">
                                        ...
                                        <div class="flex gap-4 mt-2 sm:mt-0">
                                            <label class="inline-flex items-center">
                                                <input type="radio" name="student_attendance[{{ $student->id }}]" value="Hadir" {{ $status === 'Hadir' ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500 border-slate-300">
                                                <span class="ml-2 text-sm text-slate-700">Hadir</span>
                                            </label>
                                            <label class="inline-flex items-center">
                                                <input type="radio" name="student_attendance[{{ $student->id }}]" value="Tidak Hadir" {{ $status === 'Tidak Hadir' ? 'checked' : '' }} class="text-rose-600 focus:ring-rose-500 border-slate-300">
                                                <span class="ml-2 text-sm text-slate-700">Tidak Hadir</span>
                                            </label>
                                        </div>
                                    </div>
                                    <div class="mt-2">
                                        <input type="text" name="student_evaluations[{{ $student->id }}]" value="{{ $evaluation }}" placeholder="Penilaian/catatan untuk {{ $student->name }} (opsional)" class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm text-slate-600">
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="report_note" id="report_note" rows="4" required class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" placeholder="Contoh: Fokus pernapasan gaya bebas, murid mengalami peningkatan...">{{ old('report_note', $trainingReport->report_note) }}</textarea>
                    </div>

                    <div class="flex justify-end gap-3 mt-8">
                        <a href="{{ route('admin.operations.recap') }}" class="px-4 py-2 border border-slate-300 rounded-lg shadow-sm text-sm font-medium text-slate-700 bg-white hover:bg-slate-50">
                            Batal
                        </a>
                        <button type="submit" class="px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 flex items-center gap-2">
                            <i class="fa-solid fa-save"></i> Update Laporan
                        </button>
                    </div>
                </form>
            </div>
