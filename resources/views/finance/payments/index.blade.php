        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6">
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                    <form action="{{ route('finance.payments.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3 items-end">

                        <div class="w-full sm:flex-1">
                            <label for="search" class="block text-sm font-medium text-slate-700 mb-1">Cari Murid</label>
                        ...
                        <div class="w-full sm:w-40">
                            ...
                            <select name="status" id="status" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        ...
                        <div class="w-full sm:w-36">
                            <label for="payment_method" class="block text-sm font-medium text-slate-700 mb-1">Metode Pembayaran</label>
                            <select name="payment_method" id="payment_method" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                                <option value="">Semua Metode</option>
                                <option value="Bank BCAS" {{ request('payment_method') === 'Bank BCAS' ? 'selected' : '' }}>Bank BCAS</option>
                                <option value="Bank BSI" {{ request('payment_method') === 'Bank BSI' ? 'selected' : '' }}>Bank BSI</option>
                                <option value="Cash" {{ request('payment_method') === 'Cash' ? 'selected' : '' }}>Cash (Tunai)</option>
                            </select>
                        </div>

                        <div class="flex gap-2 w-full sm:w-auto">
                            <button type="submit" class="w-full sm:w-auto bg-slate-800 hover:bg-slate-900 text-white px-5 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm flex items-center justify-center gap-2">
                                <i class="fa-solid fa-filter"></i> Filter
                            </button>
                            @if(request()->hasAny(['search', 'status', 'month', 'year', 'payment_method']))
                                <a href="{{ route('finance.payments.index') }}" class="w-full sm:w-auto bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center">
                                    Reset
                                </a>
                            @endif
                        </div>
