        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl p-8 border border-slate-100">
                ...
                <div class="mb-8 border-b border-slate-100 pb-5">
                    <h3 class="text-lg font-bold text-slate-800"><i class="fa-solid fa-briefcase mr-2 text-blue-500"></i> Formulir Jabatan Baru</h3>
                    <p class="text-sm text-slate-500 mt-1">Lengkapi informasi jabatan, nominal gaji/honor, dan status pengaktifan jabatan ini.</p>
                </div>

                <form method="POST" action="{{ route('master.positions.store') }}" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <div class="input-group">
                            ...
                            <x-text-input id="name" class="block w-full" type="text" name="name" :value="old('name')" required autofocus placeholder="Contoh: Pelatih Kepala" />
                        </div>
                    ...
                    <div>
                        ...
                        <div class="input-group">
                            ...
                            <x-text-input id="base_salary" class="block w-full" type="number" name="base_salary" :value="old('base_salary')" placeholder="Contoh: 3000000" min="0" />
                        </div>
                    ...
                    <div>
                        ...
                        <div class="input-group">
                            ...
                            <textarea id="description" name="description" rows="3" placeholder="Jelaskan peran dan tanggung jawab untuk jabatan ini (Opsional)...">{{ old('description') }}</textarea>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="input-group">
                            ...
                            <select id="status" name="status">
                                <option value="aktif" {{ old('status', 'aktif') === 'aktif' ? 'selected' : '' }}>Aktif (Dapat Digunakan)</option>
                                <option value="nonaktif" {{ old('status') === 'nonaktif' ? 'selected' : '' }}>Nonaktif (Tidak Dapat Digunakan)</option>
                            </select>
                        </div>
                        <p class="text-xs text-slate-400 mt-1.5">Jabatan nonaktif tidak akan muncul sebagai opsi untuk ditautkan ke pengguna.</p>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-6 border-t border-slate-100 mt-8">
                        ...
                        <button type="submit" class="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white font-bold text-sm rounded-xl transition-all duration-200 shadow-md shadow-slate-800/20 hover:shadow-lg hover:shadow-slate-800/30 active:scale-95">
                            <i class="fa-solid fa-floppy-disk mr-1"></i> Simpan Jabatan
                        </button>
                    </div>
                </form>
