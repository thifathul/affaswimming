        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl p-8 border border-slate-100">
                ...
                <div class="mb-8 border-b border-slate-100 pb-5">
                    <h3 class="text-lg font-bold text-slate-800"><i class="fa-solid fa-user-pen mr-2 text-blue-500"></i> Perbarui Profil Murid</h3>
                    <p class="text-sm text-slate-500 mt-1">Ubah data murid, kontak wali murid, atau perbarui tautan akun login yang terhubung.</p>
                </div>

                <form method="POST" action="{{ route('master.students.update', $student->id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="input-group">
                            ...
                            <x-text-input id="name" class="block w-full" type="text" name="name" :value="old('name', $student->name)" required autofocus placeholder="Nama Lengkap Siswa" />
                        </div>
                    ...
                    <div class="mb-5">
                        ...
                        <div class="input-group">
                            ...
                            <x-text-input id="birth_place_date" class="block w-full" type="text" name="birth_place_date" :value="old('birth_place_date', $student->birth_place_date)" required placeholder="Contoh: Bandung, 12 Agustus 2010" />
                        </div>
                    ...
                    <div class="mb-5">
                        ...
                        <div class="input-group">
                            ...
                            <x-text-input id="age" class="block w-full" type="number" min="1" name="age" :value="old('age', $student->age)" required placeholder="Contoh: 10" />
                        </div>
                    ...
                    <div class="mb-5">
                        ...
                        <div class="input-group">
                            ...
                            <x-text-input id="school" class="block w-full" type="text" name="school" :value="old('school', $student->school)" required placeholder="Nama Sekolah (Contoh: SDN 1 Bandung)" />
                        </div>
                    ...
                    <div class="pt-4 border-t border-slate-100 mt-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
                            @forelse($swimClasses as $swimClass)
                                @php
                                    $isChecked = (is_array(old('swim_class_ids')) && in_array($swimClass->id, old('swim_class_ids'))) || 
                                                 (!old('_token') && in_array($swimClass->id, $selectedClasses));
                                @endphp
                                <label class="flex items-start gap-3 p-2 rounded-lg hover:bg-slate-100 cursor-pointer transition-colors">
                                    <input type="checkbox" name="swim_class_ids[]" value="{{ $swimClass->id }}" {{ $isChecked ? 'checked' : '' }} class="mt-0.5 rounded border-slate-300 text-blue-600 shadow-sm focus:ring-blue-500 w-5 h-5">
                                    <div>
                    ...
                    <div class="pt-4 border-t border-slate-100 mt-6">
                        <h4 class="text-md font-bold text-slate-800 mb-4"><i class="fa-solid fa-key text-blue-500 mr-2"></i> Akun Login & Status</h4>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="input-group">
                                    ...
                                    <x-text-input id="email" class="block w-full" type="email" name="email" :value="old('email', $student->user ? $student->user->email : '')" placeholder="nama@email.com" />
                                </div>
                                <p class="text-xs text-slate-400 mt-1.5">Ubah email login, atau isi untuk membuat akun baru.</p>
                                <x-input-error :messages="$errors->get('email')" class="mt-2 text-xs text-red-500" />
                            </div>

                            <!-- Status -->
                            <div>
                                <label for="status" class="block text-sm font-semibold text-slate-700 mb-1.5">Status Murid</label>
                                <div class="input-group">
                                    <i class="fa-solid fa-toggle-on"></i>
                                    <select id="status" name="status">
                                        <option value="aktif" {{ old('status', $student->status) === 'aktif' ? 'selected' : '' }}>Aktif (Bisa Login)</option>
                                        <option value="nonaktif" {{ old('status', $student->status) === 'nonaktif' ? 'selected' : '' }}>Nonaktif (Tidak Bisa Login)</option>
                                    </select>
                                </div>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-6 border-t border-slate-100 mt-8">
                        ...
                        <button type="submit" class="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white font-bold text-sm rounded-xl transition-all duration-200 shadow-md shadow-slate-800/20 hover:shadow-lg hover:shadow-slate-800/30 active:scale-95">
                            <i class="fa-solid fa-rotate mr-1"></i> Perbarui Murid
                        </button>
                    </div>
                </form>
